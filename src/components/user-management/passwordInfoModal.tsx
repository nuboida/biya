...
interface PasswordInfoModalProp {
  onClose: () => void;
  password: string;
}

export const PasswordInfoModal = ({
  onClose,
  password,
}: PasswordInfoModalProp) => {
  return (
    <InfoModal title="" onModalClose={() => onClose()} actionName="Close" action={() => onClose()}>
      ...
      <div className="flex flex-col justify-center items-center">
        <h1 className="text-2xl font-semibold mb-5">New Password</h1>
        <p>{password}</p>
      </div>
    </InfoModal>
  );
